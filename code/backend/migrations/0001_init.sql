...
--
-- 本文件是 data/ddl.sql 的镜像,作为 sqlx 迁移在**服务启动时自动执行**
-- (DbPools::migrate → sqlx::migrate!),使部署无需额外 data 阶段即可建表。
-- 变更表结构时:不要编辑本文件(已发布迁移不可改),新增 0002_*.sql;
-- 并同步更新 data/ddl.sql,保持两份一致。
--
-- 依据:
